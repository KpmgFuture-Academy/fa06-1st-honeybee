...
### 10-1) 전처리 후 생성 파일 목록
- `CropParameters_clean.csv`
- `GreenhouseClimate_clean.csv`
- `GrodanSens_clean.csv`
- `LabAnalysis_clean.csv`
- `Production_clean.csv` *(2버전: 전체 / 수확 직전 6일 분석용)*
- `Resources_clean.csv`
- `TomQuality_clean.csv`
- `Weather_clean.csv` *(원본 `Weather.csv`에서 변환)*

### 10-2) 주요 전처리 항목
- **결측치 처리**
  - 연속형 데이터: 선형 보간(linear interpolation)
  - 범주형 데이터: 최빈값 대체(mode imputation)
  - 분석에 영향이 큰 경우 → 제거 대신 보간 우선 적용
- **이상치 처리**
  - IQR(사분위 범위) 기반 ±1.5×IQR 이상 값 제거 또는 보간
  - 물리적으로 불가능한 값(음수 생산량, 비현실적 온도 등) 제거
- **데이터 타입 변환**
  - 날짜/시간 → `datetime64[ns]` 변환
  - 수치형 오브젝트 컬럼 → `float64` 변환
  - 불필요한 공백·문자 제거
- **시계열 정리**
  - 시간 순서 정렬
  - 불규칙 간격 → 일정 간격(분/시간)으로 리샘플링 및 보간

---

## 11. 분석용 파생 변수 및 피처 생성

### 11-1) `features_stage_week.csv`
- 스테이지(대분류) × **7일 주차**(소분류)로 묶어 **평균/합계/표준편차** 및 **주차별 GDD 합·누적** 수록  
- **용도**: “단계·주차별 어떤 조건(VPD/DLI/EC/WC/CO₂/관수/에너지)이 성능에 유리했나?”

### 11-2) `Production_last6days.csv`
- **수확일 D0 기준 D-6~D-1**의 환경·배지·날씨·자원 **요약통계(평균/표준편차/최소/최대/합)**와 당일 수확량을 결합  
- **용도**: “수확 직전 6일 조건이 수확량/등급에 어떤 영향을 줬나?”

### 11-3) `targets_daily.csv`
- 일단위로 **생산(Prod__) / 품질(Qual__)** 타깃을 정리  
- **용도**: 상관분석/모델링의 **타깃 테이블**

### 11-4) `setpoints_recommend.csv`
- 스테이지별 **GDD 누적 상위 25% 주차**에서 **핵심 매개변수의 중앙값(IQR)**을 권장범위로 제안  
- **용도**: MCU **세트포인트 후보값**(VPD/DLI/EC/WC/관수/에너지 등) 초기 설정 가이드
